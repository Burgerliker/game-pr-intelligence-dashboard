...
const SIGNALS = [
  {
    label: "감성 논조",
    weight: "50%",
    body: "부정어·긍정어 키워드 사전으로 기사마다 점수를 매기고, 얼마나 확실하게 부정인지를 가중해 평균을 냅니다.",
  },
  {
    label: "기사량 급증",
    weight: "25%",
  ...
  },
  {
    label: "위험 주제",
    weight: "15%",
    body: "확률형·규제·환불·운영 장애·여론 악화 등 PR 위기 유형별로 가중치를 달리 줍니다.",
  },
  {
    label: "보도 채널",
    weight: "10%",
    body: "조선·연합·KBS 등 종합 매체 보도일수록 높은 가중치를 줍니다.",
  },
];
          ...
          <Paper sx={{ ...sectionCardSx, background: "linear-gradient(135deg,#0f172a 0%,#111827 55%,#1f2937 100%)", p: { xs: 3, md: 5 } }}>
            <Stack spacing={3}>
              ...
              <Stack direction="row" spacing={1.5} alignItems="center" sx={{ pt: 0.5, borderTop: "1px solid rgba(255,255,255,.07)" }}>
                <Box>
                  <Typography sx={{ fontWeight: 700, fontSize: 13, color: "#e2e8f0" }}>문종원</Typography>
          ...
          <Paper sx={{ ...sectionCardSx, p: { xs: 2.5, md: 4 } }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3,1fr)" }, gap: 2 }}>
              {[
                ...
                { icon: "📡", title: "매체가 파급력을 결정한다", body: "파급력 있는 조선·연합뉴스 등의 매체에 가중치를 더했습니다." },
          ...
          <Paper sx={{ ...sectionCardSx, p: { xs: 2.5, md: 4 } }}>
            ...
            <Stack spacing={1}>
              {[
                ...
                { title: "EMA 동적 α 평활화", body: "평활 계수(α)를 동적으로 조절해 급등 구간에서는 α를 낮춰(0.1) 노이즈를 흡수하고, 초기값에는 α=1.0을 적용해 첫 점수를 정확히 반영했습니다." },
                { title: "버스트 조기 복귀 조건 추가", body: "30분간 위험도 55점 미만이 유지되면 조기 복귀해 불필요한 수집 빈도를 줄였습니다." },
              ].map((item) => (
